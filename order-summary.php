...
$yy = "SELECT inv_id, SUM(price*Qty) AS tot FROM live_cart WHERE $whr=$user_id AND flag!='0'";
...
WHERE live_cart.".$whr." =  '$user_id' AND scat.deli_method =  '1' AND live_cart.flag!='0'
...
$sqlw = "SELECT MAX(books.wrap_charges) AS wrap_charges FROM books Inner Join scat ON books.sccode = scat.sccode Inner Join live_cart ON books.itemcode = live_cart.Item_Code WHERE live_cart.".$whr." =  '$user_id' AND flag!='0' AND scat.wrapping = '1'";
$resultw = mysqli_query($conn, $sqlw);
$rowdeli = mysqli_fetch_array($resultw);

$wp = $rowdeli['wrap_charges'];
...
$sqldel = "SELECT MAX(books.deli_charges) AS deli_charges FROM books Inner Join live_cart ON books.itemcode = live_cart.Item_Code WHERE live_cart.".$whr." = '$user_id' AND flag!='0' AND books.deli_charges!= 250";
...
<div class="slider-area">
							...
							<div class="slider-container">
									...
									<div class="accordion-inner no-p table-responsive">
										<table width="100%" border="0" cellpadding="14" class="table table2" style="margin-bottom: 0;">
											...
											<?php $sql_c = mysqli_query($conn, "SELECT
												...
												live_cart
												WHERE
												live_cart.$whr =  $user_id AND flag!='0'");$sub = 0;
											while($row_c = mysqli_fetch_array($sql_c)) 
											{
												$itm_code = $row_c['Item_Code'];

												
												$sql_del = "SELECT scat.deli_method, books.sccode, books.deli_charges FROM books Inner Join scat ON books.sccode = scat.sccode
												...
												<?php 
													...
													// $deli_charges = $row2['deli_charges'];
													 } 
												?>
												<tr>
													<td colspan="3" align="right">
														...
														<?php if($countw !=0){?>
															...
															<td align="right">
																...
																<h5>LKR <span id="del_fee" data-value="<?php echo $deli_char; ?>"><?php echo number_format($deli_char, 2); ?></span></h5>
																<h5>LKR <span id="pro_fee">0.00</span></h5>
																<?php 
																	if($countw !=0){
																?>
																	<h5>LKR <span id="wrapping" data-value="<?php echo $wrap; ?>"><?php echo number_format($wrap, 2); ?></span></h5><br>
								...
								<form method="post" action="https://www.payhere.lk/pay/checkout" id="form_sandbox" style="display: none;">
									<!-- <form method="post" action="https://sandbox.payhere.lk/pay/checkout" id="form_sandbox" style="display:none"> -->  <!--SandBox URL  --> 
										<input type="text" name="merchant_id" value="212597" readonly>
										<!-- <input type="hidden" name="merchant_id" value="1211943" readonly> --> <!-- SandBox ID -->
										<input type="text" name="return_url" value="https://www.cellparts.lk/order-confirm" readonly>
										<input type="text" name="cancel_url" value="https://www.cellparts.lk/order-cancel" readonly>
										<input type="text" name="notify_url" value="https://www.cellparts.lk/connection/payment_verify" readonly>
										<br><br>Item Details<br>
										<input type="text" name="order_id" id="order_id" value="<?php echo $inv_id; ?>" readonly>
										<input type="text" name="items" value="CellParts.lk" readonly><br>
										<input type="text" name="currency" value="LKR" readonly>
										<input type="text" name="amount" value="<?php echo $g_tot; ?>" id="g_tot_amnt" readonly>
										<br><br>Customer Details<br>
										<input type="text" name="first_name" value="<?php echo $sqltmp['fname']; ?>" readonly>
										<input type="text" name="last_name" value="<?php echo $sqltmp['lname']; ?>" readonly><br>
										<input type="text" name="email" value="<?php echo $sqltmp['email']; ?>" readonly>
										<input type="text" name="phone" value="<?php echo $sqltmp['Mob_no']; ?>" readonly><br>
										<input type="text" name="address" value="<?php echo $sqltmp['address']; ?>" readonly>
										<input type="text" name="city" value="<?php echo $sqltmp['town']; ?>" readonly>
										<input type="text" name="country" value="Sri Lanka" readonly><br><br>    
									</form>
  ...
  <script>
  	...
  	$("#btn_order").click(function() {
  		...
  		var additional_details = $('#additional_details_cr').val();
  		...
  		if (tot_amnt > 0) {
  			...
  			if(chk) {
  				...
  				var del_fee =  parseFloat($('#del_fee').attr('data-value')).toFixed(2);
  				
  				$.post('connection/payment_confirm.php', {'up_payment_method': 'data', inv: inv, pay_type: '1', del_type: del_type, del_fee:del_fee, packaging:packaging, additional_details:additional_details}, function (data) {
  	...
  	$("#btn_order_bank").click(function() {
  		...
  		var packaging =  parseFloat($('#wrapping').attr('data-value')).toFixed(2);
  		var del_fee =  parseFloat($('#del_fee').attr('data-value')).toFixed(2);
  		var additional_details = $('#additional_details_bk').val();
  		...
  		if (tot_amnt > 0) {
  			...
  			if(chk) {
  				...
  				$.post('connection/payment_confirm.php', {'payment_confirm': 'data', del_type: del_type, packaging:packaging, del_fee:del_fee, additional_details:additional_details}, function (data) {
  					if (data.status === 'success') {
  						var inv = data.inv_id;
