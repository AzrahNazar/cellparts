...
include 'db/db.php';
$chk=isset(htmlspecialchars(trim($_GET['exist']), ENT_QUOTES, 'UTF-8')) ? htmlspecialchars(trim($_GET['exist']), ENT_QUOTES, 'UTF-8') : ''; 

 $id = isset(htmlspecialchars(trim($_GET['id']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['id']), ENT_QUOTES, 'UTF-8'): '0';
 $key = isset(htmlspecialchars(trim($_GET['key']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['key']), ENT_QUOTES, 'UTF-8'): '0';
...
$section =isset(htmlspecialchars(trim($_GET['sel']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['sel']), ENT_QUOTES, 'UTF-8') : '';
$first_name = isset(htmlspecialchars(trim($_GET['dip']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['dip']), ENT_QUOTES, 'UTF-8'): '';
$scat = isset(htmlspecialchars(trim($_GET['scat']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['scat']), ENT_QUOTES, 'UTF-8'): '';
$cats =isset( htmlspecialchars(trim($_GET['cats']), ENT_QUOTES, 'UTF-8'))?htmlspecialchars(trim($_GET['cats']), ENT_QUOTES, 'UTF-8'): '';
$combo =isset( htmlspecialchars(trim($_GET['combo']), ENT_QUOTES, 'UTF-8'))? htmlspecialchars(trim($_GET['combo']), ENT_QUOTES, 'UTF-8'): '';
...
<?php include "common/mobilemenu.php"; ?>
    ...
    margin-left: 2px; height: auto; padding-bottom: 15px; padding-right: 12px;">
							...
							<img src="images/sumeda_p/<?php echo $arr['imgpath'];?>" style="padding-top:30px;width:350px;" id="myImg" alt="<?php echo $arr['book'];?>">
							<input id="pid" type="hidden" class="form-control" value="<?php echo $arr['itemcode'];?>" style="width:250px">
							...
							<h4 class="sss" style="margin-left: 12px; margin-bottom: -15px;"><?php echo $arr['book'];?></h4>
							
							</div>
							<hr>
							<div class="row" style="margin-left: 1px; margin-bottom: -15px; margin-top: -15px;">
							<span class="price product-price price_rs">Rs <?php echo $arr['price'];?></span>
												<span class="price product-price price_usd">$ <?php echo number_format($arr['price']/$dollar,2);?></span>
							</div><hr>
							<div class="row">
							
							<h5 class="sss" style="margin-left: 12px; text-align: justify; margin-right: 5px; margin-bottom: -10px; margin-top: -10px;"><?php echo $arr['details'];?></h5>
...
$(document).ready(function () {
    $(".pro-add-to-cart").click(function () {
        ...
        //cart_data_load();
        $(this).replaceWith( "<a class='button pro-add-to-cart' title='Added to cart' type='button' style='font-size: 13px;color:#ffffff;'><span><i class='fa fa-shopping-cart' style='font-size: 13px;color:#ffffff;'></i> Added to Cart</span></a>" );
