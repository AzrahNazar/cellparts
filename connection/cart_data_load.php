...
if (array_key_exists('cart_data_load', $_POST)) {//data table
	...
	if (!isset($_SESSION['user'])) {
		...
		while ($rowu = mysqli_fetch_assoc($resultu)) {
			...
			$sql = "SELECT * FROM live_cart WHERE tmp_id = '".htmlspecialchars(trim($_SESSION['temp_user']), ENT_QUOTES, 'UTF-8')."'  AND flag!='0' ORDER BY id DESC" ;
		...
		while ($rowu = mysqli_fetch_assoc($resultu)) {
			...
			if($user != ''){
				...
				$sql = "SELECT * FROM live_cart WHERE User_ID = '$user_id' AND flag!='0' ORDER BY id DESC" ;
				$result = mysqli_query($conn,$sql);
...
if (array_key_exists('cart_data_load_show', $_POST)) {//data table
		...
		$resultu = mysqli_query($conn,$sqlu);
			...
			while ($rowu = mysqli_fetch_assoc($resultu)) {
				if($user != ''){
					$sql = "SELECT * FROM live_cart WHERE tmp_id = '".$user."' AND flag!='0' ORDER BY id DESC" ;
					$result = mysqli_query($conn,$sql);
