...
$sql_t = "INSERT INTO order_details (inv_id, User_ID, fname, lname, Tel_no, Mob_no, email, address, town, zip, status, guest, date, order_date, g_total, pay_method, delivery_type, additional_details) SELECT inv_id, User_ID, fname, lname, Tel_no, Mob_no, email, address, town, zip, status, guest, date, '$date', g_total, pay_method, delivery_type, additional_details FROM order_details_temp WHERE inv_id='$order_id' LIMIT 1";
		
if(mysqli_query($conn, $sql_t)) {

	$sqldel = "SELECT MAX(books.deli_charges) AS deli_charges FROM books Inner Join live_cart ON books.itemcode = live_cart.Item_Code WHERE inv_id='$order_id'";
	...
	/*********************** CUSTOMER SMS ****************************/
	$Mob_nos = mysqli_fetch_assoc(mysqli_query($conn, "SELECT Mob_no FROM order_details WHERE inv_id='$order_id'"));
	$mobile_no = $Mob_nos['Mob_no'];
	$inv_idd = $order_id;
		 ...
		 $data_sms = array(
		 	...
		 	'msg' => "Your order has been successfully placed. Thank you! Order ID: ".$inv_idd, //Message
	...
	/*********************** OWNER SMS ****************************/
	 ...
	 $data_sms = array(
	 	...
	 	'email' => 'sumedacellular@gmail.com', //User Email
	 	'pwd' => 'Sum66556', //User Password
	 	'msg' => "New order has been received. Order ID is ".$inv_idd, //Message
