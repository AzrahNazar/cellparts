...
$countu=mysqli_num_rows($resultu);
	...
	if($countu==1){
		...
		$query = mysqli_query($conn,"SELECT bid FROM home_newar WHERE name='$name' OR bid='$bid'");
		...
		if (mysqli_num_rows($query) > 0){
			...
			function GetImageExtension($imagetype)
			 ...
			 if (!empty($_FILES["file"]["name"])) {
				...
				$file_name = htmlspecialchars(trim($_FILES["file"]["name"]), ENT_QUOTES, 'UTF-8');
				//$_FILES["file"]["name"];
				...
				$target_path = "../../images/access/".$final_image;
				  ...
				  if(move_uploaded_file($temp_name, $target_path)) {
					 ...
					 $query_upload="INSERT INTO home_newar SET name='$name', price='$price', link='',	img='$final_image',bid='$bid', isbn='', details='$details', pub='', bp='', wprice='$wprice',img_old='', new_arr = '$new_arr'";
